...
import { X, Droplets, Calendar, TrendingUp, BarChart3 } from 'lucide-react'
import { t } from '../i18n'
import { getWeekSummary } from '../utils/statsHelpers'
import { getWeekKey } from '../utils/gamification'

/**
 * Statistik-Modal: Tagesverlauf Wasser (7/14/30 Tage), Wochenvergleich, Aktivität pro Woche.
 */
...
export default function StatsModal({
  ...
  return (
    ...
    >
      <div
        className="bg-app-surface rounded-card-lg shadow-modal dark:shadow-modal-dark max-w-md w-full max-h-[90vh] overflow-hidden border border-app-border animate-slide-up flex flex-col"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="flex items-center justify-between p-4 border-b border-app-border-subtle">
          <h2
            id="stats-modal-title"
            className="text-lg font-semibold text-[var(--app-primary)] flex items-center gap-2"
          >
          ...
          <button
            type="button"
            onClick={onClose}
            className="p-2 rounded-button text-app-muted-foreground hover:bg-app-surface-hover hover:text-[var(--app-primary)] transition-colors"
        ...
        <div className="p-4 overflow-y-auto space-y-6">
          {/* Tagesverlauf Wasser (Balken) + Tage-Auswahl */}
          ...
          {/* Aktivität pro Woche (letzte 6 Wochen) */}
